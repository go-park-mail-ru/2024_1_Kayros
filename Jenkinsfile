      ...
      for (int i = 0; i < microservices.length; i++) {
          stage("Build Microservice: ${microservices[i]}") {
                script {
                  ...
                  sh "sudo docker build -t resto-${microservices[i]}-service:latest -f /var/lib/jenkins/workspace/resto-backend/integration/microservices/integration/microservices/${microservices[i]}/Dockerfile ."
                }
          }
      }
