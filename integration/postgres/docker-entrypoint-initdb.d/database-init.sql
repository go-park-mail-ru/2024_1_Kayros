...
=======
    ...
    phone       TEXT CONSTRAINT user_phone_domain CHECK (LENGTH(phone) = 18) NULL, -- формат |+7 (989) 232 12 12|
    password    BYTEA CONSTRAINT user_password_length CHECK (LENGTH(password) = 64) NOT NULL,
    address     TEXT CONSTRAINT user_address_length CHECK (LENGTH(address) BETWEEN 14 AND 100) NULL,     -- |ул. Мира, д. 4| (самое короткое название улицы в Москве 4 символа)
    img_url     TEXT CONSTRAINT user_img_url CHECK(LENGTH(img_url) BETWEEN 20 AND 60) DEFAULT '/minio-api/users/default.jpg' NOT NULL,
    card_number BYTEA CONSTRAINT user_card_number CHECK (LENGTH(card_number) = 64) NULL,
