...
const (
	time    = 1        // specifies the number of passes over the memory
	memory  = 2 * 1024 // specifies the size of the memory in KiB
	threads = 2
	keyLen  = 56
...
func HashData(saltProps []byte, plainPassword string) []byte {
	...
	copy(salt, saltProps)
	hashedPassword := argon2.IDKey([]byte(plainPassword), salt, time, memory, threads, keyLen)
