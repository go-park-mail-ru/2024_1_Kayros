...
func NewMetrics(reg prometheus.Registerer, namespace string) *Metrics {
	...
	reg.MustRegister(m.NumberOfSpecificRequests)
	reg.MustRegister(m.RequestTime)
	reg.MustRegister(m.MicroserviceErrors)
	reg.MustRegister(m.MicroserviceTimeout)
	reg.MustRegister(m.DatabaseDuration)
	reg.MustRegister(m.PopularRestaurant)
	reg.MustRegister(m.PopularFood)
	reg.MustRegister(m.PopularCategory)
	return m
}
